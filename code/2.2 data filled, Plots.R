...
ggplot(data = df, aes(x = pm2.5_Normalized...16, fill=Station.name, colour=Station.name)) +
  geom_density(alpha=0.4, size=0.5) +
  geom_density(aes(x = pm2.5_Normalized...32, group=Station.name),
               inherit.aes = FALSE,
               alpha=0.4) +
...
p2_Vis <- df %>%
  ...
  geom_line(aes(x=Visibility_deNormalized), stat = "density", col = "red", linetype= "3313") +
  theme_ipsum()
p2_Vis
