ggplot(data, aes(x = Month, y = Day, fill = pm10)) +
  geom_tile() +
  scale_fill_gradient(low = "white", high = "red") +
  labs(x = "Column", y = "Row", title = "Example Heatmap") +
  facet_grid(~Station.name)
...
df <- daily_data %>% mutate(year = year(Date),
...
#create plotting df
df <-df %>% select(Station.name,day,month,year,pm10)%>%
  fill(pm10) #optional - see note below
...
p <-ggplot(df,aes(month,day,fill=pm10))+
